<?php 
    ...
    $post_action_title = 'Chia sẻ file';
    $post_action = 'Posting';
    $post_id = '';
    if(isset($post[0]))
    {
    ...
    <div class="container">
        <div class="question__cover">
            <div class="question__content">
                <h1>Viết bài</h1>
                <form action="<?php echo base_url('FileUpload/'.$post_action) ?>" id="question_form" method="POST">
                    ...
                    <label for="">File<i class="fas fa-asterisk asterisk--rule"></i></label>
                    <?php if (isset($error['tags'])) { echo "<div class='notify-error'>".$error['tags']."</div>";}?>
                    <input type="file" name="file" placeholder="Hỏi đáp, chia sẻ, ..." value="<?php echo $post_tag ?>">
                    <label for="">Nội dung<i class="fas fa-asterisk asterisk--rule"></i></label>
                    <div class="textarea">
                        <div class="textarea__buttons">
                            <button title="Strong text"><i class="fas fa-bold"></i></button>
                        ...
                        <textarea form="question_form" name="content" id="question_textaria" placeholder="Nội dung (nên dùng chức năng trên thanh chức năng-bên trên hoặc thẻ HTML để định dạng văn bản)."><?php echo $post_content ?></textarea>
                    </div>
